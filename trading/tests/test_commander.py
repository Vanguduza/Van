...
def env(tmp_path):
    ...
    runner = FakeRunner()
    st = CommanderSettings(token=TOKEN, ledger=str(tmp_path / "l.sqlite"), heartbeat_dir=str(hb), log_dir=str(tmp_path / "log"), data_dir=str(data), vekl_url="http://127.0.0.1:1", runner=runner, accounts_registry=str(tmp_path / "accounts.json"), secrets_dir=str(tmp_path / "secrets"))
