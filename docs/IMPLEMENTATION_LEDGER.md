...
- **VATI build G — trade preview:** `vati/app/tradebook.py` read model (past / current / potential trades from the ledger, recent refusals only) and `vati/arbiter/confidence.py` (uncalibrated reduce-only confidence score with bands, logged per candidate in `OPPORTUNITY_ASSESSMENT`); gateway `GET /v1/trading/trades`; Android `trading/TradeBook.kt` parser + `OverlayMode.TRADES` panel with tabs and confidence chips; `VanGatewayClient.tradingTrades`. Evidence: 321 tests passed (root pytest); Kotlin `TradeBookTest` 6/6 via kotlinc + JUnit (Android SDK absent in the build container, app compile pending on device toolchain).
- **VATI builds H–J — production infrastructure, deployment and the Trading Command Center (Rev 5, `docs/VAN_TRADING_PRODUCTION_DEPLOYMENT_BLUEPRINT_REV5.md`):** account registry with credential references and safety identity; PostgreSQL ledger (byte-identical chain, row-locked head, tested on PostgreSQL 16); Deriv WebSocket and MT5 mTLS transports with the Windows MT5 bridge worker (mTLS, HMAC, SL-only, tighten-only; end-to-end tested); Dukascopy/Deriv/CSV feeds and a hash-manifested bar lake; Tier-1 calendar loader (two sources for live eligibility); session service with ACCOUNT_SNAPSHOT / MARKET_DATA_HEALTH events, heartbeat, owner-halt observation and safety-identity refusals; dedicated trading VEKL service (vendored DIAL resolver with provenance, activation store); Hermes subordinate commander (typed, signed, allowlisted, MCP stdio shim) and its profile registration; `deploy/van-trading-core` bootstrap/qualify/systemd/Supabase/PKI; Command Center read models (`/v1/trading/portfolio|accounts|market-state|risk|trades/{id}|bars`) and the Android Trading Command Center (overview, trades, trade workspace with chart/levels/timeline, instrument workspace, Risk Center, accounts; pure-Kotlin models and chart geometry tested with kotlinc). Evidence: 340 pytest, Node 4/4, Kotlin 13/13; bootstrap dry-run, env generator, PKI and Hermes splice exercised in the container. Android compile and the VM itself remain external gates.
...
- VATI Rev 5 is a Development Authority Candidate until owner acceptance (A4). Builds A–J are complete on synthetic data and container-verifiable infrastructure; the VM bootstrap/qualify run, the Windows MT5 worker host, account plug-in, real market data on the VM and the Android device build are owner/external gates (Rev 5 Part K). Builds A–F are complete on synthetic data; real-data backtests (Parquet lake, Dukascopy/TrueFX), the NautilusTrader donor gate (Python 3.12), PostgreSQL ledger, live MT5 bridge worker, demo trading with transports, ZSE broker verification (VATI-ZSE-F001) and any LIMITED_LIVE mandate remain external gates. Adapters exist as fail-closed contracts without live transports.
