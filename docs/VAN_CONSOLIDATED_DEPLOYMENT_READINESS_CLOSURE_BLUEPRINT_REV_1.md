...
| 6 | `computer_use` fabric | **Keep, and make the missing executor a runtime refusal.** *Corrected during execution: the Rev 2 default said delete, on the description "a stub". Reading the module showed that description wrong — the refusals are real, ordered and tested, and the ledger is durable; what it lacks is a worker. Deleting a hardened boundary because the thing it bounds is unbuilt gets the order backwards, and re-deriving it later under delivery pressure is how a `RUN_ARBITRARY` appears. `SURFACE_WORKERS` is empty, `begin` refuses `OPERATION_NO_WORKER_FOR_SURFACE` before writing anything, and `/v1/computer-use/health` reports it (P2-CU-001).* | Delete the package, its tests, the `computer_operations` table, its retention policy and the `OperationState` mapping in `coherence/owner_status.py`. |
