...
| `evidence/van-system-audit/component_ledger.json` | 209 components, all at a terminal state: 132 integrated, 70 externally blocked, 7 deliberately removed | `tools/ci/maturity_gate.py`, `tools/ci/ledger_reconcile.py` — the first refuses a ledger that overstates, the second one that understates |
