...
## Remote Browser Rev 1.5 — §43 production acceptance, 2026-09-20

**Verdict: REPOSITORY_COMPLETE_PENDING_EXTERNAL.** §43 names one hundred and four requirements. Sixty are proven by the repository and forty-four are not, and the forty-four are not a backlog — thirty-seven need a Browser Stream Host, a carrier or the owner's handset, and seven need the owner to provision or accept something.

The verdict above is **recomputed** by `tools/ci/production_acceptance.py` from `evidence/van-system-audit/production_acceptance.json`, and the checker fails if this document's verdict disagrees with its own rows. That is deliberate: a verdict written at the top of a ledger by hand is written once and never re-derived.

`REPOSITORY_PROVEN` is the strongest thing a repository can say and it is weaker than certification. It means the repository demonstrates the behaviour and names what does. It does not mean anyone has watched it work on a phone.

| §43 group | Proven | Blocked external | Owner deployment |
|---|---|---|---|
| Authority | 7 | 0 | 0 |
| Runtime | 1 | 7 | 0 |
| Native experience | 2 | 4 | 1 |
| VAN symbiosis | 5 | 3 | 0 |
| Governance | 6 | 0 | 0 |
| Reliability | 3 | 4 | 0 |
| Performance | 0 | 5 | 0 |
| Certification | 4 | 2 | 0 |
| VAN⇄Hermes continuity | 9 | 0 | 0 |
| Offline voice | 4 | 2 | 3 |
| Native Android browser behavior | 6 | 2 | 2 |
| Owner-S24 zero-config binding | 9 | 3 | 0 |
| Stream/control topology | 0 | 5 | 1 |
| Programme/ledger coherence | 4 | 0 | 0 |
| **Total** | **60** | **37** | **7** |

Per-requirement statuses, evidence and the named blocker for each unproven one are in the register. Every `REPOSITORY_PROVEN` row cites a file that exists, and a citation naming a Python test names one that can be run — the checker resolves the `Class::test` chain through the module's AST rather than looking for the name, because a name that occurs in a file is not a test anyone can re-run.

### What the forty-four are waiting for

- **14×** — a provisioned Browser Stream Host running Chromium and the media pipeline (RB-010)
- **11×** — a Browser Stream Host and the owner's S24 Ultra (RB-010, Gate 14)
- **7×** — the owner's S24 Ultra (Gate 14); no repository test substitutes for a handset
- **3×** — CI, the only authority that compiles the Android app module
- **1×** — a real carrier handover (Gate 14)
- **1×** — a signed voice asset pack the owner supplies; no ASR model ships (RB-073/RB-078)
- **1×** — a signed voice asset pack the owner supplies (RB-073/RB-078)
- **1×** — an Android system TTS engine with offline voice data installed on the handset
- **1×** — a private VCN and issued client certificates (RB-115/RB-116)
- **1×** — a host with interfaces to scan (RB-117)
- **1×** — the Trading Core host's deployment, which is the owner's
- **1×** — a real TLS handshake, in either direction (RB-115)
- **1×** — a cross-host control path (RB-010/RB-115)

The §38 red-team matrix is tracked separately in `evidence/van-system-audit/red_team_register.json`: sixty-six scenarios, fifty-seven `PASS`, nine `BLOCKED_EXTERNAL`, none `assumed`.
