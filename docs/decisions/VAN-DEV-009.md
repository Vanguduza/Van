# VAN-DEV-009 — Typed actions, ApprovalSheet flows, STALE_VIEW
...
**Depends on:** VAN-DEV-001, 006, 007.
**Branch:** `claude/busy-bardeen-sbfo2u-van-android` (not merged, not pushed).

## Scope (`VAN-DEVCC-R1` §3.3, §5)

Pure half `dialdev/DialDevActions.kt`; Compose half `command/dev/DevActions.kt`.

| ID | Requirement | Proven by |
|---|---|---|
| DEV9-R1 | Every action is `POST /v1/dial-dev/actions` through `postProved` with `idempotency_key` and `expected_projection_revision`; no revision on screen → nothing sent | `DialDevActionsTest` · `every action carries…`, `no revision on screen means no action…` |
| DEV9-R2 | No optimistic success: 202 → `ACCEPTED · pending` (`LiveBadge`) until the projection's `actions[]` shows that `action_id` APPLIED / REJECTED(reason) / SUPERSEDED; an `APPLIED` in the POST answer is not trusted | `202 is ACCEPTED and pending…`, `only the projection moves an accepted action…` |
| DEV9-R3 | `409 STALE_VIEW` → refetch and show the new state before a retry; that retry is a new decision (new key) | `409 is STALE_VIEW carrying the new revision…` |
| DEV9-R4 | A send whose connection dropped is `Unconfirmed`; its retry reuses the key | `an unconfirmed send retries with the same key…` |
| DEV9-R5 | REJECTED shows its reason inline; no action caption claims "passed"/"done" or reads `favourable` | `a rejection shows its reason inline`, `no action caption ever claims a pass` |
| DEV9-R6 | Destructive = `REVOKE_TASK` and reject-`DECIDE`; both through `ApprovalSheet` with the consequence **from projection data** (DIAL's own `consequences.REVOKE_TASK` sentence, else lease id + uncommitted-file count, saying which it did not report); revoke confirmed by typing the task id; reject requires a reason | `revoke and reject are destructive…`, `the revoke consequence prefers DIAL's own sentence…` |
| DEV9-R7 | Steer sheet shows the task's current intent and next action **above** the input; the text is guidance DIAL Hermes relays, never terminal input | `DevSteerSheet` |
| DEV9-R8 | Offline: owner actions are refused with the reason, never queued | `DevActions.submit` (projection `online` flag); reducer `Offline(0)` |

`ApprovalSheet` gained two optional, backward-compatible parameters (`approveEnabled`,
`confirmation` slot) for the typed confirmation and the required reason; existing call sites are
unchanged. The sheet closes only on ACCEPTED; any other outcome is shown inside it.

## Open item for the owner

Revoke is guarded by the hardware device proof (`postProved`) and a typed confirmation, not by a
biometric prompt. If revoke should also require `BiometricGate` (as A4 command approvals do), that
is an owner decision this unit does not take.
