# VAN-DEV-011 — V6 seven-state previews, V7 accessibility, V8 coherence evidence
...
**Depends on:** all VAN-DEV units.
**Branch:** `claude/busy-bardeen-sbfo2u-van-android` (not merged, not pushed).

## Scope (`VAN-DEVCC-R1` §8, DNA §5/§7)

- **V6**: `android/visual-preview/.../VanDevControlCentreStates.kt` renders, for every one of the
  fifteen `VanRoute.DEV_TEMPLATES`, a sheet of all seven states (LOADING, CONTENT, EMPTY, ERROR,
  DEGRADED, OFFLINE, STALE). Each tile is the **shipping** `DialDevScreenReducer`'s output for a
  preview fixture parsed by the **shipping** parser for that route — fixtures live only in
  `visual-preview` (DNA §5). `renderVanPreviews` writes them to
  `artifacts/release/preview/dev-control-centre/*_seven_states.png` (15 files).
  `VanDevControlCentreStatesTest` asserts every template has a fixture, every tile classifies to
  the state it claims, the sheets render and file names are distinct.
- `DegradedCatalog` gains `dial_dev`, `orca`, `spmrf`, `openviking`, `vekl`, `artemis`, `zuul`,
  `hermes_dial` (`DialDevEnvelopeTest` · `DegradedCatalog names every DIAL development subsystem`).
- **V7**: every screen is a `LazyColumn` (list-first); every pressable row goes through
  `VanPressable` (≥ 48 dp) with a content description; chips announce their label; the graph is
  fully navigable as a list; terminal lines announce redactions; nothing under 12 sp (`type.label`
  is the floor; monospace uses `type.data` 13 sp).
- **V2 self-review** (lint + reading): no `Color(0x…)`, no `N.sp`, no `Card(` in new files
  (`android_design_lint.py`: 0 new violations); status roles only through exhaustive enum `when`s;
  every panel's KDoc declares `@DataSource`; no fixture path under `app/src/main`.
- **V8**: the rendered sheets were reviewed by the implementing agent only; an independent V8
  coherence review against the anti-generic checklist is still owed.

## Proof

`VanDevControlCentreStatesTest` (4 tests) in the standalone `visual-preview` run (28/28 passed,
baseline 24/24). The in-tree `./gradlew :visual-preview:test` could not run here (AGP) — CI runs it.
