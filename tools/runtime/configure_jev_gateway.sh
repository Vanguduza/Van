...
if [[ -n "$OWNER" ]]; then
  chown "$OWNER" "$CONFIG_ROOT/jev-projection.token" "$CONFIG_ROOT/jev-control.token" "$GATEWAY_ENV"
fi

tmp="$(mktemp "$CONFIG_ROOT/gateway.env.XXXXXX")"
trap 'rm -f "$tmp"' EXIT
grep -Ev '^VAN_JEV_(ENABLED|BASE_URL|PROJECTION_TOKEN_FILE|CONTROL_TOKEN_FILE|TIMEOUT_SECONDS)=' "$GATEWAY_ENV" > "$tmp" || true
cat >>"$tmp" <<ENV
