...
def cmd_record_acceptance(args):
    ...
    append_receipt(
        ...
        _receipt(
            ...
            [sha,final["contract_sha256"],final["identity_spec_sha256"],final["visual_acceptance_matrix_sha256"],str(final.get("key_id") or ""),str(final.get("verified_by") or "")],
            ["OWNER_ACCEPTED"],
            args.actor,
...
def cmd_release(args):
    ...
    if release_path.is_file() and status.get("qual_emb_01")=="READY":
        ...
        if existing.get("rive_sha256")==sha and existing.get("contract_sha256")==sha256_file(CONTRACT_PATH) and existing.get("identity_spec_sha256")==sha256_file(IDENTITY_DOC) and existing.get("visual_acceptance_matrix_sha256")==sha256_file(VISUAL_ACCEPTANCE_MATRIX):
            print("NO_CHANGE"); return 0
    release={"asset":rel(APP_RIV),"source_asset":rel(SOURCE_RIV),"artboard":"Van","state_machine":"VanRuntime","git_sha":_git_head(),"rive_sha256":sha,"contract_sha256":sha256_file(CONTRACT_PATH),"identity_spec_sha256":sha256_file(IDENTITY_DOC),"visual_acceptance_matrix_sha256":sha256_file(VISUAL_ACCEPTANCE_MATRIX),"visual_authority_revision":"2.3","rive_android":str((((tools.get("critical_path") or {}).get("rive_android") or {}).get("version"))),"rive_authoring_tool":"rive_cli","rive_authoring_version":str((((tools.get("critical_path") or {}).get("rive_cli") or {}).get("version"))),"emulator_validation_run":(status.get("production") or {}).get("ci_run"),"device_checklist":rel(DEVICE_PATH),"acceptance":rel(ACCEPTANCE_PATH)+"#final","released_at":now_iso()}
