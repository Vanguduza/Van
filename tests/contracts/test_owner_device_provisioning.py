...
def test_the_two_screens_that_carried_the_forms_have_no_editable_field_at_all():
    ...
    DNA §4's rebuild split the old `command/modules/AdminModules.kt` (`ConnectionsModule` /
    `SystemsModule` / `SettingsModule`) into two DNA destinations: `command/connected/
    ConnectedRoute.kt` (Google planes, Hermes, knowledge readiness) and `command/settings/
