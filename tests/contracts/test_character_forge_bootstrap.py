...
def test_as_forge_switches_identity_without_recursing():
    qualifier = _text("qualify-netcup-authoring.sh")
    block = qualifier[qualifier.index("as_forge() {"):qualifier.index("\n}\n", qualifier.index("as_forge() {")) + 3]
    assert 'runuser -u "$FORGE_USER" -- "$@"' in block
    assert 'else\n    as_forge "$@"' not in block
